...
% [B2-5] wall-cooling (Robin BC) parameters, set in grenade06.m
global TAMB XMAX YMAX h_wall lam_wall

% --- velocities: no-slip on all outer walls --------------------------------
uw_prev = u(2,1:NPJ+2);     % [B2] save west-face history BEFORE the wall zeroing:
                            % the vent update below must under-relax against the
                            % PREVIOUS face value (see comment there). Zeroing
                            % first made u(2,Jv) = -relax_vent*u_orf every call,
                            % i.e. the vent passed only 30% of the orifice-law
                            % velocity (verified V2b: uvent 10.6 = 0.3*33.9 m/s).
...
% --- convective wall loss (Robin BC), west/top/bottom ------------------[B]
% [B2-5] ACTIVATED per UPDATE_2_mixing_outputs.md item 1 (was the designer's
% commented-out block; h_wall/lam moved to grenade06.m parameters, h_wall = 0
% recovers the adiabatic walls above). Casing heat loss q = h*(T - TAMB):
%   T_ghost = (T_int + Bi*TAMB)/(1+Bi),  Bi = h*spacing/(2*lambda_gas)
% East wall stays adiabatic (insulated by the unburnt charge, design doc
% sec. 1); the vent rows stay zero-gradient (outflow carries its T out).
if h_wall > 0.
    BiX = h_wall*(XMAX/NPI)/(2*lam_wall);  BiY = h_wall*(YMAX/NPJ)/(2*lam_wall);
