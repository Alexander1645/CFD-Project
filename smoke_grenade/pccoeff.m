...
for I = Istart:Iend
    ...
    for J = Jstart:Jend
        ...
        % Transient compressible continuity: net mass flux in must equal the
        % rate of mass accumulation plus the gas GENERATED by combustion.
        %   (rho-rho_old)/Dt*Vol + (Fe-Fw+Fn-Fs) = Sgen*Vol
        % Two sources are added to the imbalance b':
        %  (1) THERMAL EXPANSION  (rho_old-rho)/Dt : a heated cell's gas expands.
        %  (2) COMBUSTION GAS     f_gas*wburn*rho : the charge consumed per second
        %      by the rate-anchored front releases f_gas of it as gas products.
        % This 2-D source uses the LOCAL GAS density on purpose. The physically-
        % correct solid-density source (~1800) is ~2000 kg/m3/s in EACH front cell,
        % a local velocity divergence ~5000 1/s that a 2 mm cell cannot store or
        % resolve (the gas blowing off a burning surface is a sub-grid feature) -
        % it sends the field to NaN regardless of the global pressure closure,
        % because the dP0/dt feedback only balances the GLOBAL mass budget, not a
        % LOCAL cell spike. The chamber pressure buildup is therefore modelled by
        % the lumped low-Mach P_chamber closure in run_grenade.m (block 1b), which
